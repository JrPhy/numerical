...
//Loading files
	FILE *fp;
	float x[m+1], y[m+1], cd[m+1];
	double A[m+1][n+1], B[n+1][n+1];
	double L[n+1][n+1], temp, max[n+1], d[n+1], maximum, c[n+1], z[n+1], Binv[n+1][n+1], CDLLS[m+1], minimum;
	fp = fopen("A_matrix.txt","r");
		if (fp == NULL) printf("A_matrix doesn't exist\n");
		for (i=1; i<m+1; i++) fscanf(fp, "%f	%f	%f\n",&x[i] ,&y[i] ,&cd[i] );
	fclose(fp);
//Loading files
//Getting matrix A
	for(i=1; i<m+1; i++) 
	{
		...
		A[i][9] = x[i]*y[i]*y[i];
		A[i][10] = y[i]*y[i]*y[i];
		A[i][11] = x[i]*x[i]*x[i]*x[i];
		A[i][12] = x[i]*x[i]*x[i]*y[i];
		A[i][13] = x[i]*x[i]*y[i]*y[i];
		A[i][14] = x[i]*y[i]*y[i]*y[i];
		A[i][15] = y[i]*y[i]*y[i]*y[i];
	}
//Getting matrix A
...
//Getting A^T*cd
	...
	for (i=1; i<m+1; i++) CDLLS[i] = 0;
	
	for (j=1; j<n+1; j++) for (i=1; i<m+1; i++) c[j] = c[j] + A[i][j]*cd[i];
//Getting A^T*cd
//Getting B^-1*A^T*cd
