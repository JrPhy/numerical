...
A[1:k,14] = x*y*y*y
A[1:k,15] = y*y*y*y
z <- qr.fitted(qr(A), cd, k = 15)
a <- qr.coef(qr(A),cd)			# calculate the coefficients of the monomial
...
for (j in 1:yy)
{
	for (i in 1:xx)
	{
		Reg_CD [j,i] = a[1] + a[2]*x[i] + a[3]*y[j] + a[4]*x[i]*x[i] + a[5]*x[i]*y[j] + a[6]*y[j]*y[j] + a[7]*x[i]*x[i]*x[i] + a[8]*x[i]*x[i]*y[j] + a[9]*x[i]*y[j]*y[j] + a[10]*y[j]*y[j]*y[j] + a[11]*x[i]*x[i]*x[i]*x[i] + a[12]*x[i]*x[i]*x[i]*y[j] + a[13]*x[i]*x[i]*y[j]*y[j] + a[14]*x[i]*y[j]*y[j]*y[j] + a[15]*y[j]*y[j]*y[j]*y[j]
	}
}
